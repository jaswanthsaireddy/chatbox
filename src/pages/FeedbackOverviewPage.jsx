...
const FeedbackOverview = () => {
  ...
  const feedbackData = pastConversations
    .map((conversation, index) => ({
      id: index,
      rating: conversation.conversationFeedback?.rating || 0,
      comment: conversation.conversationFeedback?.comment?.trim() || "No comment"
    }))
    .filter(item => filterRating === "all" || item.rating === Number(filterRating))
...
export default React.memo(FeedbackOverview);
